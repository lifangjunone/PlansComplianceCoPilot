...
function getPartition(): BytecloudJwtPartition {
  const partition = import.meta.env.VITE_PARTITION

  if (!partition) {
    throw new Error('VITE_PARTITION is required to initialize SSO authentication.')
  }
...
export function getCurrentUserApiUrl(
  ...
  if (matchedHost) {
    return `https://${matchedHost}${CURRENT_USER_API_PATH}`
  }

  return `https://${DEFAULT_CURRENT_USER_API_HOST}${CURRENT_USER_API_PATH}`
}
...
export class AccessDeniedError extends Error {
  ...
  constructor(owner: string) {
    super('Current user does not have permission to access this page.')
    this.name = 'AccessDeniedError'
    this.owner = owner
  }
}

async function getAccessDeniedOwner(response: Response): Promise<string | null> {
  try {
    ...
    // Ignore malformed 401 bodies and let the caller treat them as auth failures.
  }
